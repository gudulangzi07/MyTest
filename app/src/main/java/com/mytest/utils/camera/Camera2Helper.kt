package com.mytest.utils.camera
...
class Camera2Helper(val mActivity: Activity, private val mTextureView: TextureView) {
    ...
    private var savePicCallback: SavePicCallback? = null

    fun setSavePicCallback(savePicCallback: SavePicCallback){
        this.savePicCallback = savePicCallback
    }

    interface SavePicCallback{
        fun savePicSuccess(path: String)
    }
...
//            mActivity.toast("相机硬件不支持新特性")
    ...
    private val onImageAvailableListener = OnImageAvailableListener {
        ...
        try{
            val file = File("${mActivity.externalCacheDir}${File.separator}Orc")
            if (!file.exists()){
                file.mkdirs()
            }

            val path = "${file.absoluteFile}${File.separator}pic.jpg"
            ...
            Handler(Looper.getMainLooper()).postDelayed({
                savePicCallback?.savePicSuccess(path)
            }, 200)
    ...
    fun takePic() {
        ...
        mCameraDevice?.apply {
            ...
            mImageReader?.surface?.let { captureRequestBuilder.addTarget(it) }
